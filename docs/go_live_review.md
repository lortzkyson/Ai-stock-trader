...
## 2. The model doesn't show a clear edge — this is the bigger blocker, and it's now been checked three times
...
- The realistic backtest initially looked *better* this time — positive expectancy, Sharpe 1.12, +7.7% return — which was a genuine reversal worth taking seriously rather than accepting at face value. A one-sample t-test on the 310 real trades came back **p=0.33** (not remotely significant), and the profit was concentrated: one symbol accounted for 29% of all gross positive P&L, and fewer than half the traded symbols (14 of 29) were net positive. That's the signature of a small, noisy sample producing a lucky-looking headline number, not a demonstrated edge. *(Operational note: this Round 2 backtest report's file was later silently overwritten by Round 3's same-day run, since the filename was date-only — the numbers above are preserved here from the console output at the time; the report-naming bug that caused this is fixed, see `scripts/run_backtest.py`.)*

**Round 3** (same 30-symbol/~14-month universe, + PO3/AMD and Fair-Value-Gap/Inverse-FVG features — a specific ICT-methodology strategy thesis, requested and implemented deliberately rather than generic feature search — and a barrier horizon shortened to 1 session to match that thesis): also found a real bug along the way (`minutes_since_open`/PO3 session-open math was computing time-since-midnight-UTC-plus-9:30 rather than time-since-9:30-ET on real UTC-timestamped data — the same UTC-vs-exchange-local bug class as Phase 2's original fix, caught via a regression test using UTC timestamps before this round's numbers were trusted). After the fix:
- Signal-level walk-forward: model win rate (25.3%) notably *higher* than the random baseline (18.2%), but expectancy came in essentially tied — slightly *worse* (0.06%/trade vs. 0.07%/trade). A higher hit rate without better expectancy means the model's extra wins are small relative to its losses — not a usable edge.
- The realistic backtest (**[reports/backtest_2026-08-11_round3_po3_ifvg.md](../reports/backtest_2026-08-11_round3_po3_ifvg.md)**) was unambiguously negative this time: expectancy -0.08%/trade, Sharpe -0.83, profit factor 0.86, total return -4.8%, and (consistent with the significance check) not statistically significant either (p=0.25) — only 8 of 26 traded symbols were net positive.

Three independent, differently-scoped attempts (different universe size, different history length, different feature sets including a specific named strategy methodology) all concluded the same thing: **this signal, whether from generic price/volume features or from PO3/IFVG-specific ICT features, isn't demonstrated to beat chance on 1-minute bars for these large-cap equities.** That's a more informative and more trustworthy result than any single attempt alone — a real edge should have shown up more clearly with a more targeted, thesis-driven feature set, not stayed flat.
...
| Model shows a demonstrated edge | **No, checked three times** — generic features (Round 1, Round 2) and a dedicated PO3/IFVG strategy build (Round 3) all indistinguishable from a random baseline; two good-looking backtest results (Round 2: p=0.33, Round 3: p=0.25) both turned out to be statistically insignificant noise on inspection |
